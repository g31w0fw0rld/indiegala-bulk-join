...
Userscript that adds a unified ticket-purchase queue and utilities to Indiegala giveaways, plus two lookup buttons on store product pages. / Userscript que añade una cola unificada de compra de boletos y utilidades a los giveaways de Indiegala, y dos botones de consulta en las fichas de la tienda.
...
![The Extra Odds dialog, asking how many tickets to queue and showing the figures the decision depends on, including how many tickets you already hold](docs/screenshot-extra-odds.png)

*Queueing **several tickets of the same giveaway** (Extra Odds). It opens from the ⚠×N badge on the card, and before you commit it lays out the figures the decision depends on: price per ticket, balance, what is left after the queue, **how many tickets you already hold in this giveaway** —read from the giveaway's own page because the listing does not publish it— how many you can afford right now, and the 50-per-giveaway cap. Two separate exits, and they are not the same button: **Queue** leaves them waiting, **Queue & run** starts buying. The risky one is the outlined button, not the one the eye lands on. And you can queue beyond your balance on purpose: those tickets wait with ⏳ until you have GalaSilver. / Encolar **varios boletos del mismo giveaway** (Extra Odds). Se abre desde el badge ⚠×N de la tarjeta, y antes de comprometerte pone delante las cifras de las que depende la decisión: precio por boleto, saldo, lo que queda descontando la cola, **cuántos boletos ya tienes comprados de ese giveaway** —leídos de su propia página porque el listado no lo publica—, cuántos te alcanzan ahora mismo, y el tope de 50 por giveaway. Dos salidas distintas, y no son el mismo botón: **Encolar** los deja esperando, **Encolar y ejecutar** empieza a comprar. La arriesgada es la del borde, no la que se lleva la vista. Y puedes encolar por encima de tu saldo a propósito: esos boletos esperan con ⏳ hasta que tengas GalaSilver.*

![The giveaway's own page, with the line at the foot that publishes how many tickets you hold](docs/screenshot-extra-odds-source.png)

*Where that figure comes from. The listing does not publish it — its "tickets sold" is everybody's total, not yours — so the only place your own count exists is the giveaway's page, in the line at the foot: **GIVEAWAY 1208242 - 10 TICKETS PURCHASED**. The 10 is drawn inside that dark circle by CSS, so the text the script reads has no parentheses around it. The script asks for this page once, when you open the queue dialog, and never for the cards of the listing: with "load every page" ticked that would be one request per card. / De dónde sale esa cifra. El listado no la publica —su "tickets sold" es el total de todo el mundo, no el tuyo—, así que el único sitio donde existe tu cuenta es la ficha del giveaway, en el renglón del pie: **GIVEAWAY 1208242 - 10 TICKETS PURCHASED**. El 10 va dentro de ese círculo oscuro por CSS, así que el texto que lee el script no lleva paréntesis. El script pide esta ficha una vez, al abrir el diálogo de encolar, y nunca para las tarjetas del listado: con "cargar todas las páginas" puesta sería una petición por tarjeta.*
...
- **Closes the tab on its own when there is nothing to show.** If the walk ends with no new prizes, the tab it opened has nothing left to read, so it counts down five seconds and closes itself. With prizes it stays open — the widget and the title badge are there to be read. If the browser refuses to close it, it says so instead of pretending it did.
...
- **Cierra la pestaña sola cuando no hay nada que enseñar.** Si el recorrido acaba sin premios nuevos, la pestaña que abrió no deja nada que leer: hace una cuenta atrás de cinco segundos y se cierra. Con premios se queda abierta —el widget y el contador del título están para leerlos—. Si el navegador se niega a cerrarla, lo dice en vez de fingir que lo hizo.
