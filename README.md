...
![The queue panel, the GalaSilver widget with the wheel countdown, the Extra Odds dialog and the buttons the script injects on indiegala.com/giveaways](docs/screenshot-giveaways.png)

*Three things at once. **Queue panel** (bottom left) with an Extra Odds giveaway queued ×10 and a row dimmed with ⏳ — that one is short on GalaSilver, so the run skips it and buys it later instead of stopping. **GalaSilver widget** (top right): balance, what the whole queue is missing, and the wheel countdown on your own clock. **The Extra Odds dialog**, with the figures it is offering before you commit —what you can afford right now, what is already queued, the 50-per-giveaway cap— and two separate exits: Queue, or Queue & run. On the cards behind: ＋ to queue a Single Ticket, ✕ in the opposite corner to hide that giveaway until it ends, ✓ for the ones already queued. / Tres cosas a la vez. **Panel de la cola** (abajo a la izquierda) con un giveaway de Extra Odds encolado ×10 y una fila atenuada con ⏳ — a esa le falta GalaSilver, así que la corrida la salta y la compra luego en vez de pararse. **Widget de GalaSilver** (arriba a la derecha): saldo, cuánto le falta a la cola entera, y la cuenta atrás de la ruleta en tu reloj. **El diálogo de Extra Odds**, con las cifras que te enseña antes de comprometerte —lo que te alcanza ahora, lo que ya está encolado, el tope de 50 por giveaway— y dos salidas distintas: Encolar, o Encolar y ejecutar. En las tarjetas de detrás: ＋ para encolar un Single Ticket, ✕ en la esquina opuesta para ocultar ese giveaway hasta que termine, ✓ en los que ya están en cola.*
...
**Wheel of Fortune**
- Watches the wheel entry in the user menu and then **asks the site every 15 minutes** —the same place Indiegala reads it from to draw its own menu— **without reloading the page**, so a change of state does not slip by; when it changes, it tells you three ways at once, none of them interrupting: a **double chime**, a **🎡 mark at the start of the tab title** — the only part you can read from another tab — and a toast that stays until you dismiss it. The mark goes away once you spin. The sound needs your first click on the page to unlock it (browsers refuse to play audio before that); from then on it plays even from another tab, because the page is never reloaded out from under it. The widget also **counts down to the next wheel** on your own clock, switching to «available now» when there is one — the hour is an assumption (00:00 UTC, when the site's day starts), because Indiegala states it nowhere, and the script logs the window in which it sees the wheel appear so the assumption can be checked.
...
**Wheel of Fortune**
- Vigila la entrada de la ruleta en el menú de usuario y después **le pregunta al sitio cada 15 minutos** —al mismo sitio del que Indiegala lo saca para pintar su propio menú— **sin recargar la página**, para que un cambio de estado no se te pase; cuando cambia, te avisa por tres vías a la vez y ninguna te interrumpe: un **doble toque de sonido**, una **marca 🎡 al principio del título de la pestaña** —lo único que se lee desde otra pestaña— y un toast que se queda hasta que lo cierres. La marca se va al girar. El sonido necesita tu primer clic en la página para desbloquearse (los navegadores no lo permiten antes); a partir de ahí suena aunque estés en otra pestaña, porque la página ya no se recarga bajo tus pies. El widget lleva además la **cuenta atrás de la próxima ruleta** en tu reloj, y pasa a «disponible ahora» cuando la hay —la hora es una suposición (00:00 UTC, el arranque del día del sitio), porque Indiegala no la publica en ninguna parte, y el script apunta en la consola la ventana en la que la ve aparecer para poder comprobarla—.
