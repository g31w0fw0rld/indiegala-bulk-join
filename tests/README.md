...
| `test-aviso-ruleta.js` | Que el aviso de «hay ruleta por girar» llegue **entero y en el orden que sirve**: el `alert()` con su texto, la marca 🎡 al principio del `<title>` **ya puesta cuando el diálogo aparece** —el arnés fotografía el título en ese instante, porque un `alert()` congela el hilo y lo que no esté pintado antes no se ve hasta cerrarlo—, y el toast que no se va solo. Incluye el control negativo —sin ruleta, título intacto, ningún toast y ningún diálogo— y el caso en el que **el sitio reescribe el título** por su cuenta, donde la marca tiene que volver encima del título nuevo. Y la **cuenta atrás de la ruleta** en el widget: que cuente cuando no la hay —con su forma, `3h 12m (18:00)`, no una plantilla con los `{v}` sin sustituir— y que pase a «disponible ahora» cuando sí. El sonido no se prueba: está desactivado, y además jsdom no reproduce nada. |
